...
public class StaffService implements Service{
    private final StaffManagement management = new StaffManagement();
...
@Override
    ...
    public void staffLogin() { // 사원 로그인
        ...
        while (true) {
            try {
                if (management.getEmployee().get(i).getStaffNumber() == inputStaffId) {
                    ui.Distinction(2);
    ...
    public void staffCreate() {   // 사원직급 선택
        management.createByStaff(inputStaffNumber(),  inputStaffName(), inputStaffSsn(),intputPosition(), "-");
        adminManagement();
    }
    @Override
    public void findAll() { //모든 사원 출력
            try {
                if (management.getEmployee().get(0) == null) {
        ...
        System.out.println(management.getEmployee().toString());
    }
    ...
    public int inputStaffNumber() {// 사원ID 만들기
        ...
        while (true) {
            ...
            try {
                ...
                for (Staff staff : management.getEmployee()) {
